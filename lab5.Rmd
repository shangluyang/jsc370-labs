...
medians <- station_ave[, .(temp_50 = quantile(temp, probs=0.5, na.rm=T), 
                           wind.sp=quantile(wind.sp, probs=0.5, na.rm=T),
...
mid_point <- met[, .(lon_50 = quantile(lon, probs=0.5, na.rm=T),
                     lat_50 = quantile(lat, probs=0.5, na.rm=T)), by = STATE]
...
met[, temp_cat := ifelse(state_temperature < 20, "low", 
                         ifelse(state_temperature < 25, "Mid", "high"))]
```
...
tab <- met[, .(N_entries=.N,
               ...
               mean_stm.press = mean(atm.press, na.rm=T)), by =temp_cat]

knitr::kable(tab, caption="summary information")
```
...
The R square for linear model is 0.01756, while the R square for spline model is 0.67 which indicates that the spline model perform better than the linear model, however, the spline model run the risk of overfitting.
